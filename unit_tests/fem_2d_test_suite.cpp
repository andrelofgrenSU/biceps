...
{
    Eigen::MatrixXd points_tri_right_deg_1 = FEM2D::reference_element_points_rs(TRIANGLE_RIGHT, 1);
    Eigen::MatrixXd points_tri_right_deg_2 = FEM2D::reference_element_points_rs(TRIANGLE_RIGHT, 2);
    Eigen::MatrixXd points_tri_left_deg_1 = FEM2D::reference_element_points_rs(TRIANGLE_LEFT, 1);
    Eigen::MatrixXd points_tri_left_deg_2 = FEM2D::reference_element_points_rs(TRIANGLE_LEFT, 2);
    Eigen::MatrixXd points_quad_deg_1 = FEM2D::reference_element_points_rs(QUADRILATERAL, 1);
    Eigen::MatrixXd points_quad_deg_2 = FEM2D::reference_element_points_rs(QUADRILATERAL, 2);
...
BOOST_AUTO_TEST_CASE(test_assemble_stiffness_matrix)
{
    ...
    mesh.extrude_z(svec_p1);
    for (int gp = 1; gp <= 5; ++gp) {
        Eigen::MatrixXd A_1x1 = Eigen::MatrixXd(
            FEM2D::assemble_stiffness_matrix(mesh, gp)
        );
        double error = (A_1x1_ref - A_1x1).cwiseAbs().maxCoeff();
        BOOST_TEST_CHECK(
             error < tol_vec[gp-1],
...
BOOST_AUTO_TEST_CASE(test_assemble_stiffness_xx_matrix)
{
    ...
    mesh.extrude_z(svec_p1);
    for (int gp = 1; gp <= 5; ++gp) {
        Eigen::MatrixXd A_xx_1x1 = Eigen::MatrixXd(
            FEM2D::assemble_stiffness_xx_matrix(mesh, gp)
        );
        double error = (A_xx_1x1_ref - A_xx_1x1).cwiseAbs().maxCoeff();
        BOOST_TEST_CHECK(
             error < tol_vec[gp-1],
...
BOOST_AUTO_TEST_CASE(test_assemble_stiffness_zz_matrix)
{
    ...
    mesh.extrude_z(svec_p1);
    for (int gp = 1; gp <= 5; ++gp) {
        Eigen::MatrixXd A_zz_1x1 = Eigen::MatrixXd(
            FEM2D::assemble_stiffness_zz_matrix(mesh, gp)
        );
        double error = (A_zz_1x1_ref - A_zz_1x1).cwiseAbs().maxCoeff();
        BOOST_TEST_CHECK(
             error < tol_vec[gp-1],
