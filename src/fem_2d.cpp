...
void FEM2D::lagrange_basis(
	...
	for (int i = 0; i < qpoints_rs.rows(); i++) {
		...
		if (cell_type == QUADRILATERAL && degree == 1) {
			...
			(cell_type == TRIANGLE_LEFT || cell_type == TRIANGLE_RIGHT)
			&& degree == 1
		) {
		...
		} else if (cell_type == QUADRILATERAL && degree == 2) {
...
Eigen::MatrixX<FloatType> FEM2D::reference_element_points_rs(
	...
	if (cell_type == TRIANGLE_LEFT)
		cell_type = TRIANGLE_RIGHT;
...
void FEM2D::gauss_legendre_quadrature(
	...
	if (cell_type == QUADRILATERAL) {
	...
	} else if (cell_type == TRIANGLE_LEFT || cell_type == TRIANGLE_RIGHT) {
		if (precision == 1) {
			points <<
		...
		// TODO: throw exception (unrecognized cell type)
	}
}

void FEM2D::gauss_legendre_quadrature_edge_quadrilateral(
	const int precision,
	const int edge_id,
	Eigen::MatrixX<FloatType> &points_ret,
	Eigen::VectorX<FloatType> &weights_ret
) {
	points_ret = Eigen::MatrixX<FloatType>::Zero(precision, 2);
	weights_ret = Eigen::VectorX<FloatType>::Zero(precision);
	Eigen::VectorX<FloatType> qpoints_1d;
	FEM1D::gauss_legendre_quadrature(precision, qpoints_1d, weights_ret);
	if (edge_id == NORTH_ID) {
		for (int q = 0; q < qpoints_1d.size(); q++) {
			points_ret(q, 0) = qpoints_1d(q);
			points_ret(q, 1) = 1.0;
		}
	} else if (edge_id == WEST_ID) {
		for (int q = 0; q < qpoints_1d.size(); q++) {
			points_ret(q, 0) = 0.0;
			points_ret(q, 1) = qpoints_1d(q);
		}
	} else if (edge_id == SOUTH_ID) {
		for (int q = 0; q < qpoints_1d.size(); q++) {
			points_ret(q, 0) = qpoints_1d(q);
			points_ret(q, 1) = 0.0;
		}
	} else if (edge_id == EAST_ID) {
		for (int q = 0; q < qpoints_1d.size(); q++) {
			points_ret(q, 0) = 1.0;
			points_ret(q, 1) = qpoints_1d(q);
		}
	} else {
		// TODO: throw exception (unrecognized edge_id)
	}
}

FloatType FEM2D::calculate_area(
	...
	for (int k = 0; k < mesh.cmat.rows(); k++) {
		...
		for (int q = 0; q < qpoints_rs.rows(); q++) {
			area += ABS_FUNC(detJ_rs(q))*qweights(q);
		}
	}
	return area;
}
...
Eigen::VectorX<FloatType> FEM2D::assemble_load_vector(
	...
	Eigen::VectorX<FloatType> load_vec = Eigen::VectorX<FloatType>::Zero(mesh.nof_dofs());
	for (int k = 0; k < mesh.cmat.rows(); k++) {
		element = mesh.cmat.row(k);
		node_coords_v = mesh.pmat(element, Eigen::all);
		...
		for (int q = 0; q < qpoints_rs.rows(); q++) {
			...
			for (int i = 0; i < mesh.dofs_per_cell(); i++) {
				 load_vec(element(i)) += phi_rs(q, i)*detJxWxf;
			}
		}
	}
	return load_vec;
	...
	for (int kr = 0; kr < nz; kr++) {
		for (int kc = 0; kc < nx; kc++) {
			...
			for (int q = 0; q < qpoints_rs.rows(); q++) {
				...
				for (int i = 0; i < mesh.dofs_per_cell(); i++) {
					 expan_mat(element_v(i), kc) += phi_rs(q, i)*(1-r)*Wxf;
					 expan_mat(element_v(i), kc+1) += phi_rs(q, i)*r*Wxf;
				}
			}
			k++;
		}
...
FloatType FEM2D::L2_norm(const FEMFunction2D &u, const Eigen::SparseMatrix<FloatType> &M)
{
	return sqrt(u.vals.dot(M*u.vals));
}

FloatType FEM2D::L2_norm(const Eigen::VectorX<FloatType> &u_vec, const Eigen::SparseMatrix<FloatType> &M)
{
	return sqrt(u_vec.dot(M*u_vec));
}
